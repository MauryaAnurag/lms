...
__webpack_require__.d(__webpack_exports__, {
  s: () => (/* binding */ createNextRouteHandler),
  H: () => (/* binding */ next_createUploadthing)
});
...
;// CONCATENATED MODULE: ./node_modules/uploadthing/dist/chunk-WPSY3MFJ.mjs
...
async function dist_pollForFileData(fileKey, callback) {
    ...
    return withExponentialBackoff(async ()=>{
        ...
        if (maybeJson instanceof Error) {
            console.error(`[UT] Error polling for file data for ${fileKey}: ${maybeJson.message}`);
            return null;
        }
...
;// CONCATENATED MODULE: ./node_modules/uploadthing/dist/chunk-BPTHKJTK.mjs
...
var chunk_BPTHKJTK_buildPermissionsInfoHandler = (opts)=>{
    return ()=>{
        const r = opts.router;
...
function chunk_BPTHKJTK_createBuilder(opts) {
    return (input)=>{
        return internalCreateBuilder({
            routerConfig: input,
            ...opts
...
;// CONCATENATED MODULE: ./node_modules/uploadthing/dist/chunk-F3SNDKZQ.mjs
...
var uploadFilesInternal = async (data, opts)=>{
    ...
    if (!res.ok) {
        const error = await dist_UploadThingError.fromResponse(res);
        throw error;
    }
    const clonedRes = res.clone();
    const json = await res.json();
    if ("error" in json) {
        const error = await dist_UploadThingError.fromResponse(clonedRes);
        throw error;
    }
    const uploads = await Promise.allSettled(data.files.map(async (file, i)=>{
        ...
        await dist_pollForFileData(key);
        return {
            key,
            url: fileUrl,
            name: file.name,
    ...
    return uploads.map((upload)=>{
        ...
        const error = dist_UploadThingError.toObject(reason);
        return {
            data: null,
            error
        };
...
var createServerHandler = (opts)=>{
    ...
    const getBuildPerms = chunk_BPTHKJTK_buildPermissionsInfoHandler(opts);
    const GET = (request)=>{
...
var extractRouterConfig = (router)=>buildPermissionsInfoHandler({
        router
    })();
...
// src/next.ts
var next_createUploadthing = (opts)=>chunk_BPTHKJTK_createBuilder(opts);
